...
%			\vspace{0.1cm}
			\visible<1->{\item \textbf{Transkription}:
			\item[] $\Rightarrow$ \textbf{Umschreiben} eines DNA-Abschnitts zu Arbeitskopien in Form\\\hspace{1.1cm}von \textbf{mRNA} (messenger RNA)}
%			\vspace{0.1cm}
			\visible<3->{
				\item \textbf{Translation}:
					\item[] $\Rightarrow$ \textbf{Übersetzen} der Basensequenz in die Aminosäuresequenz mit\\\hspace{1.1cm}Hilfe von \textbf{tRNA} (transfer RNA)
					\vspace{-0.05cm}}
				
				\visible<4->{
					\item[] $\Rightarrow$ die tRNA besitzt eine \textbf{mRNA-Bindungsstelle} bestehend aus\\\hspace{1.1cm}einem Basentriplett
					\vspace{-0.05cm}}
				
				\visible<4->{
					\item[] $\Rightarrow$ die tRNA trägt zudem die nach dem genetischen Code \\\hspace{1.1cm}\textbf{korrespondierende Aminosäure} (AS) ihrer mRNA-\\\hspace{1.1cm}Bindungsstelle
					\vspace{-0.05cm}}
				\visible<6->{
				   \item[] $\Rightarrow$ von der Startsequenz ausgehend werden die tRNAs mit \\\hspace{1.1cm}komplementärer Bindungsstelle nacheinander an die mRNA \\\hspace{1.1cm}gebunden, dadurch wird ihre AS gelöst und an die AS der \\\hspace{1.1cm}nachfolgenden tRNA gebunden
				   \vspace{-0.05cm}
				   \item[] $\Rightarrow$ es entsteht eine \textbf{Aminosäuresequenz}}
		}
	\end{enumerate}
	\end{block}
\end{onlyenv}
...
\begin{frame}
	...
	\begin{onlyenv}<1,3-4,6>
		\begin{block}{}
			Natürlicher Vorgang der \textbf{Vervielfältigung} der DNA bei der Zellteilung:
			\begin{enumerate}
				{\setlength{\itemindent}{1.5em}
					%			\vspace{0.1cm}
					\visible<1->{\item \textbf{Topoisomerasen}: die DNA wird entwunden
					\visible<2->{\item \textbf{Topoisomerasen}: die DNA wird entwunden
					\visible<3->{\item \textbf{Topoisomerasen}: die DNA wird entwunden
					\visible<4->{\item \textbf{Topoisomerasen}: die DNA wird entwunden
					\visible<5->{\item \textbf{Topoisomerasen}: die DNA wird entwunden
					\visible<6->{\item \textbf{Topoisomerasen}: die DNA wird entwunden
				}
			\end{enumerate}
		\end{block}
	\end{onlyenv}
...
\begin{frame}
	\frametitle{Mutationen und Varianten}
	text
\end{frame}

%%Folie 6
\section{Biologischer Hintergrund}
\subsection{PCR und Sequenzierung}
\begin{frame}
	\frametitle{PCR und Sequenzierung}
	text
\end{frame}
